...
通知渠道走抽象层 .notifiers（Stage 2 prep，镜像 .fetchers 的 BaseFetcher/FetcherRegistry）：
- Notifier(ABC) + NotifierRegistry（register/get）；
- 邮件发送逻辑（smtplib SMTP_SSL 465 + app password）已移至 EmailNotifier（注册名 "email"）；
- 本模块经 NotifierRegistry.get("email") 发送，行为不变（无 creds 仍 dry-run 打印）。
SMTP 配置走 env（不进代码/日志/提交，R7/secret 红线）。
"""
...
def send_email(subject: str, body_plain: str, body_html: str,
    ...
    """（向后兼容）邮件发送——委托 NotifierRegistry.get("email")。新代码直接用 registry。
    行为不变：无 creds/收件人 → dry-run 打印（见 EmailNotifier.send）。"""
    return NotifierRegistry.get("email").send(
        to or "", subject, body_plain, body_html=body_html, log=log)
...
def notify(user_id: str, summaries: list[dict], store: ThesisStore, *,
    ...
    sent = NotifierRegistry.get("email").send(
        "", brief["subject"], brief["body_plain"],     # to 留空 → EmailNotifier 回退 env MAIL_TO（与原 send_email 默认一致）
        body_html=brief["body_html"], log=log)
    brief["sent"] = sent
    return brief
