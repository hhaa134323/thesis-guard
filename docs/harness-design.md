...
评估过：DeepEval、Arize Phoenix、Braintrust、Langfuse。

它们解决的核心问题是规模与协作——大批量 trace 存储、多人标注队列、
版本对比看板、CI 集成。本项目单人、55 条样本、本地运行，不存在这些问题。

不匹配的三点：
1. 被评对象是多轮对话收敛后的终态卡片，不是单次 input→output，
   套进标准 evaluator 需要大量胶水。
2. 需要盲评（打乱来源隐藏模型身份），标注对象是我自己，
   现成框架的标注流程不支持来源隐藏。
3. L1 确定性断言 + W2 人工判定的双层结构，现成框架多为单层。

若样本量增长到数百条或引入第二个标注者，Phoenix 的 trace 存储值得重新评估。