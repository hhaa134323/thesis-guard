...
**逐字段接受率（2026-08-03 收尾补：口径从单一总分改为逐字段呈现）**：

| 字段 | 接受率 | 失败率 | 不合格数 | 主要根因 |
|---|---|---|---|---|
| mirrors | 25/25 (100%) | 0% | 0 | — |
| holding_reason_raw | 4/5 (80%) | 20% | 1 | MCO 混入「策略：逆势、越跌越买」操作策略（非买入理由） |
| key_assumptions | 18/25 (72%) | 28% | 7 | 估值机械（EPS/FCF/DCF/SBC/倍数）误当假设（§7.3 根因；P2 落地四条定义 + 拒绝规则） |
| **合计** | **47/55 (85.45%)** | 14.55% | 8 | |

**95% CI**（Wilson，n=55，p=0.8545）≈ **[76%, 95%]**。85.45% 超 85% 门槛仅 0.45 个百分点，**过线幅度小于一个字段**（一个字段 = 1/55 ≈ 1.82pp）——翻一个字段的接受结论即跌破 85%（46/55=83.6%）。故 85.45% 不应读作「宽过线」；**key_assumptions 28% 失败是主要拖累**，P2 已落地四条定义 + 确定性 backstop（`is_paraphrase`），待重跑验证（本轮不重跑 eval，等 key_assumptions 定义经作者目检）。
