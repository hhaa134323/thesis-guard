...
9. **仓库转 public / 对外分享 / 作品集展示前，必须移除或脱敏**：`assets/`、`evals/ground_truth.yaml`、`evals/blind_verdicts.yaml`、`evals/blind_pairs.yaml`、`evals/w2_converged_cards.yaml`、`evals/w2_blind_verdicts.yaml`、`data/thesis.db`（SQLite store，含真实持仓卡）、SK 海力士真实运行 transcript/记录（2026-08-02 真实持仓）、及任何含真实持仓/真实 thesis/真实金额的文件（含新 transcript 样本）；**新增此类文件时必须同步更新本清单**
