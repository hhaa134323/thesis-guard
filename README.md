...
> 桌面 localhost 单页（对话 + 确认卡）。用户用自然对话告诉 AI「我为什么持有这只票」，AI 追问并把破局条件整理成确认卡；之后由核对 Agent 在数据更新时用一手公开披露自动核对这些条件。命中当天单独发邮件，未命中合并进每日简报。**判断权永远归用户。**
...
- **extract / menu 走 deepseek**（Phase 5 移植）：`extract_card` / `generate_menu` 工具内部用 OpenAI Agents SDK + deepseek（`submit_extraction` / `submit_menu` tool call 提交结构化输出，不用 `output_type`——避 B4 thinking 冲突 + 短路空结构）。`pydantic-ai` / `glm` / `entry_agent.py` / `menu.py` / `llm.py` 已删；prompts + `MenuMirror`/`MenuCandidates` + `filter_executable_mirrors` 移入 `orchestrator.py`。W1 eval 用 `model_override` 在同一百炼端点头对头跑 deepseek vs glm 确认抽取不退。
...
| R9 | 仓库转 public / 对外分享 / 作品集展示前，必须移除或脱敏真实持仓文件 | `assets/`、`evals/ground_truth.yaml`、`evals/blind_verdicts.yaml`、`evals/blind_pairs.yaml`、`evals/w2_converged_cards.yaml`、`evals/w2_blind_verdicts.yaml`、及任何含真实持仓/thesis/金额的文件；新增此类文件须同步更新本清单 |

红线源自复用资产 `pre-market-briefing/README.md` 的「AI 边界（硬规则）」表（4 条可以 vs 5 条不可以），原样沿用并扩充。

## 技术与分发约束

- 桌面 localhost 单页（对话+确认卡）：FastAPI + 单 HTML + .bat 启动（用户不碰 shell）；2026-08-02 形态定稿，见 `docs/PRD.md` §11 沿革。不上应用商店、不做小程序
- 部署中立：配置一律走环境变量（不写死 localhost/127.0.0.1、不留本机绝对路径），启动脚本与主程序解耦，目录按「一个命令可容器化」组织；本轮只跑本地、不加登录层
- 前端栈：React + Vite + shadcn/ui（2026-08-02 修订，作废「单 HTML+原生 JS 无构建链」）；构建产物静态文件由 FastAPI 托管，部署中立不变。详见 `docs/frontend-design-v1.md`
...
- v1 锁美股，不做 A 股/港股
- 触达仅邮件：命中条件当天单独发；未命中合并进每日简报一行带过
